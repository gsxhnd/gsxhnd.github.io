...
function searchFromKeyWord(keyword = "") {
  ...
  if (!searchJson) return -1;
    ...
    searchJson.forEach((item) => {
      ...
      if (
        lowerTitle.indexOf(handleKeyword) !== -1 ||
        lowerContent.indexOf(handleKeyword) !== -1
      ) {
        let resultItem = {};
        resultItem.title = title.replace(
          keyword,
        ...
        while (lowerContent.indexOf(handleKeyword) !== -1) {
          let begin =
            lowerContent.indexOf(handleKeyword) - sildeWindowSize / 2 < 0
              ? 0
              : lowerContent.indexOf(handleKeyword) - sildeWindowSize / 2;
          let end = begin + sildeWindowSize;
          let reg = caseSensitive
            ? new RegExp("(" + keyword + ")", "g")
            : new RegExp("(" + keyword + ")", "ig");
          resultItem.content.push(
            "..." +
              content
                .slice(lastend + begin, lastend + end)
                .replace(reg, "<span class='red'>$1</span>") +
              "..."
          );
...
if (window.isPost) {
  ...
  if (!toc || !toc.children || !toc.children[0]) {
      ...
      function getArrayFromOl(ol) {
        ...
        ol.forEach((item) => {
          if (item.children.length === 1) {
            ...
            let concatArray = getArrayFromOl(
              Array.from(item.children[1].children)
            );
            nameSet.add(
              item.children[0].getAttribute("href").replace(/^#/, "")
            );
            result.push({
              value: [
                item.children[0].getAttribute("href").replace(/^#/, ""),
              ].concat(
                concatArray.reduce((p, n) => {
                  p = p.concat(n.value);
                  return p;
                }, [])
              ),
    ...
    function reLayout() {
      let scrollToTop =
        document.documentElement.scrollTop || window.pageYOffset; // Safari is special
      if (tocToTop === 0) {
      ...
      for (let item of nameArray) {
        let dom =
          ...
          document.getElementById(item.replace(/\s/g, ""));
        if (!dom) continue;
